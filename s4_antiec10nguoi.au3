...
; Mo tiec
While 1
	Local $dangnhap = dangnhap($accMotiec, $x_main, $y_main, $lgctPos)
	If $dangnhap == False Then
		resetGame($lgctPos)
		ContinueLoop
	Else
		antiec10nguoi(0, $masoTiec, $x_main, $y_main, $lgctPos)
		ExitLoop
	EndIf
WEnd
...
While ($i < 10)
	; Dang nhap
	Local $dangnhap = dangnhap($accList[$i], $x_main, $y_main, $lgctPos)
	If $dangnhap == False Then
		resetGame($lgctPos)
		ContinueLoop
	Else
		; An tiec
		antiec10nguoi($i + 1, $masoTiec, $x_main, $y_main, $lgctPos)
		$i = $i + 1
	EndIf
WEnd
...
While 1
	...
	If $dangnhap == False Then
		...
		ContinueLoop
	Else
		antiec10nguoi(11, $masoTiec, $x_main, $y_main, $lgctPos)
		ExitLoop
	EndIf
WEnd