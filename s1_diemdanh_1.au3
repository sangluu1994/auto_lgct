...
While ($i1 < $accCloneChinhListLength)
	...
	If $dangnhap == False Then
		...
		ContinueLoop
	EndIf
	Local $thuhoach = thuhoachCloneChinh($x_main, $y_main)
	If $thuhoach == False Then
		resetGame()
		ContinueLoop
	EndIf
	Local $thinhan = thinhan($x_main, $y_main)
	If $thinhan == False Then
		resetGame()
		ContinueLoop
	EndIf
	Local $sungbai = sungbai("khonglaychinhtich", $x_main, $y_main)
	If $sungbai == False Then
		resetGame()
		ContinueLoop
	EndIf
	Local $diemdanh = diemdanhhangngay($x_main, $y_main)
	If $diemdanh == False Then
		resetGame()
		ContinueLoop
	EndIf
	Local $phattu = phattu($x_main, $y_main)
...
While ($i2 < $accCloneChanhNguListLength)
	Local $thuhoach = dangnhap($accCloneChanhNguList[$i2], $x_main, $y_main)
	If $thuhoach == False Then
		resetGame()
		ContinueLoop
	EndIf
	Local $thuhoach = thuhoachCloneChanhNgu($x_main, $y_main)
	If $thuhoach == False Then
		resetGame()
		ContinueLoop
	EndIf
...
While ($i3 < $accCloneThuongListLength)
	Local $thuhoach = dangnhap($accCloneThuongList[$i3], $x_main, $y_main)
	If $thuhoach == False Then
		resetGame()
		ContinueLoop
	EndIf
	Local $thuhoach = thuhoach($x_main, $y_main)
	If $thuhoach == False Then
		resetGame()
		ContinueLoop
	EndIf
	...
	If $thinhan == False Then
		...
		ContinueLoop
	EndIf
	Local $sungbai = sungbai("colaychinhtich", $x_main, $y_main)
	If $sungbai == False Then
		resetGame()
		ContinueLoop
	EndIf
